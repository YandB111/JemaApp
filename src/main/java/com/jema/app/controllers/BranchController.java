...
public class BranchController extends ApiController {
	...
	public ResponseEntity<?> updateBranch(@PathVariable(name = "id", required = true) Long id,
		...
		Branch mBranch = branchService.findById(id);
		...
		if (mBranch != null) {
			...
			BeanUtils.copyProperties(branchDTO, branch);
			branch.setId(id);
			branch.setCreateTime(mBranch.getCreateTime());
			branch.setUpdateTime(new Date());

			Long mID = branchService.save(branch);
			branchDTO.setId(mID);
			return new ResponseEntity<GenericResponse>(
					genericResponse.getResponse(branchDTO, "Branch successfully Updated", HttpStatus.OK),
					HttpStatus.OK);

		} else {
			return new ResponseEntity<>(genericResponse.getResponse("", "Invalid Branch", HttpStatus.OK),
					HttpStatus.OK);
		}
