...
public class VendorController extends ApiController {
	...
	public ResponseEntity<?> add(@Valid @RequestBody VendorDTO vendorDTO) {
		...
		try {
			...
			// Set a more appropriate error message for vendor conflict
			customResponse.setMessage("A vendor with the same email already exists.");
	...
	public ResponseEntity<?> update(@PathVariable(name = "id", required = true) Long id,
	    ...
	    if (mVendor != null) {
	        ...
	        try {
	            Long mID = vendorService.update(vendor); // Use the update method from the service
	            vendorDTO.setId(mID);
	            return new ResponseEntity<GenericResponse>(
