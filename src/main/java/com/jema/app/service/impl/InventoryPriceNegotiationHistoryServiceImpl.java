...
import com.jema.app.repositories.InventoryPriceNegotiationHistoryRepository;
import com.jema.app.service.InventoryPriceNegotiationHistoryService;
import com.jema.app.utils.AppUtils;

@Service
public class InventoryPriceNegotiationHistoryServiceImpl implements InventoryPriceNegotiationHistoryService {
	...
	@Autowired
	InventoryPriceNegotiationHistoryRepository inventoryPriceNegotiationHistoryRepository;

	@Override
	public Long save(InventoryPriceNegotiationHistory inventoryPriceNegotiationHistory) {
		// TODO Auto-generated method stub
		return inventoryPriceNegotiationHistoryRepository.save(inventoryPriceNegotiationHistory).getId();
	}
	...
	public List<PriceHistoryListView> findAll(String id, PageRequestDTO pageRequestDTO) {
		...
		String baseBuery = "select count(*) over() as total, i.price as price, i.total_price as total_price, i.createtime as createtime, "
				...
				+ "from inventory_price_negotiation_history i " + "left join chemical c on c.id = i.chemical_id ";
		if (pageRequestDTO.getKeyword() != null && !pageRequestDTO.getKeyword().trim().isEmpty()) {
			baseBuery = baseBuery + " where i.inventory_request_id='" + id + "' and c.code ilike '%"
					+ pageRequestDTO.getKeyword().trim() + "%'";
		} else {
			baseBuery = baseBuery + "  where i.inventory_request_id='" + id + "'";
		}
